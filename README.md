<!-- capsule-render를 활용한 다이내믹 그라데이션 헤더 -->
# Welcome to My GitHub! 👋
<img src="https://capsule-render.vercel.app/api?type=waving&color=auto&height=180&section=header&text=안녕하세요,%20백엔드%20개발자%20최준입니다&fontSize=40&animation=fadeIn" width="100%">

<br/>

## 💫 About Me
> **"기술의 본질을 이해하고, 신뢰할 수 있는 백엔드 인프라를 설계합니다."**
* 🛠️ 대용량 트래픽 처리와 비즈니스 로직 최적화에 관심이 많습니다.
* ✍️ 배운 지식을 기록하고 공유하는 것을 좋아하며, 함께 성장하는 협업 환경을 지향합니다.
* 💬 궁금한 점이 있거나 협업 제안은 언제든 환영합니다!

<br/>

## 🛠️ Tech Stack

### Languages
<img src="https://img.shields.io/badge/Java-007396?style=for-the-badge&logo=openjdk&logoColor=white"> <img src="https://img.shields.io/badge/Kotlin-7F52FF?style=for-the-badge&logo=kotlin&logoColor=white">

### Frameworks & Libraries
<img src="https://img.shields.io/badge/Spring Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"> <img src="https://img.shields.io/badge/Spring Data JPA-6DB33F?style=for-the-badge&logo=spring&logoColor=white"> <img src="https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge&logo=hibernate&logoColor=white"> <img src="https://img.shields.io/badge/QueryDSL-0769AD?style=for-the-badge&logoColor=white"> <img src="https://img.shields.io/badge/Spring Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white"> <img src="https://img.shields.io/badge/JUnit5-25A162?style=for-the-badge&logo=junit5&logoColor=white"> <img src="https://img.shields.io/badge/Swagger-85EA2D?style=for-the-badge&logo=swagger&logoColor=black">

### Databases
<img src="https://img.shields.io/badge/MySQL 8.0-4479A1?style=for-the-badge&logo=mysql&logoColor=white"> <img src="https://img.shields.io/badge/H2 Database-007396?style=for-the-badge&logoColor=white"> <img src="https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white"> <img src="https://img.shields.io/badge/Flyway-CC0200?style=for-the-badge&logo=redhat&logoColor=white">

### DevOps & Infrastructure & Tools
<img src="https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white"> <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"> <img src="https://img.shields.io/badge/AWS EC2-FF9900?style=for-the-badge&logo=amazonec2&logoColor=white"> <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"> <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"> <img src="https://img.shields.io/badge/GitHub Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white"> <img src="https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white"> <img src="https://img.shields.io/badge/k6-7D64FF?style=for-the-badge&logo=k6&logoColor=white"> <img src="https://img.shields.io/badge/Prometheus-E6522C?style=for-the-badge&logo=prometheus&logoColor=white"> <img src="https://img.shields.io/badge/Grafana-F46800?style=for-the-badge&logo=grafana&logoColor=white">

### Collaboration
<img src="https://img.shields.io/badge/Notion-000000?style=for-the-badge&logo=notion&logoColor=white"> <img src="https://img.shields.io/badge/Slack-4A154B?style=for-the-badge&logo=slack&logoColor=white"> <img src="https://img.shields.io/badge/Jira-0052CC?style=for-the-badge&logo=jira&logoColor=white">

<br/>

## 🚀 Experience & Capabilities
> 각 스택별 실무 및 프로젝트 적용 경험입니다. (접어서 보기 가능)

<details>
<summary><b>💻 Language & Framework</b></summary>

* **Java & Kotlin**: Kotlin의 간결한 문법과 Java의 객체지향 패러다임을 이해하고 적재적소에 활용합니다.
* **Spring Boot**: Layered Architecture 기반의 REST API 설계 및 비즈니스 로직을 구축합니다.
* **Spring Data JPA & QueryDSL**: 객체 중심 데이터 접근 방식을 사용하며, 동적 쿼리 및 성능 최적화(N+1 문제 해결 등)에 QueryDSL을 적극 도입합니다.
* **Spring Security & JWT**: 무상태(Stateless) 기반 인증/인가 체계를 구축하고 보안 정책을 적용합니다.
* **JUnit5**: Mockito를 결합하여 비즈니스 레이어의 단위 테스트 및 통합 테스트를 신뢰도 높게 검증합니다.
</details>

<details>
<summary><b>💾 Database & Storage</b></summary>

* **MySQL 8.0**: 테이블 스키마 설계 및 대용량 조인 성능 튜닝이 가능합니다.
* **H2 Database**: 로컬 및 개발 검증 단계에서 인메모리 DB를 사용하여 빠른 테스트를 수행합니다.
* **Redis**: 데이터 캐싱 및 분산 락(Distributed Lock) 활용을 통한 동시성 제어 경험이 있습니다.
* **Flyway**: DB 스키마 형상 관리를 자동화하여 개발-운영 환경 일치율을 유지합니다.
</details>

<details>
<summary><b>☁️ DevOps & Monitoring</b></summary>

* **Docker**: 컨테이너화를 통해 가상 인프라 구축 및 개발 서버 표준화를 주도합니다.
* **AWS EC2**: 리눅스 인스턴스 인프라를 구축하고 외부 접속 포트 및 보안 그룹을 제어합니다.
* **GitHub Actions**: CI/CD 파이프라인을 구축하여 자동 빌드, 테스트 및 원격 배포 자동화를 구현했습니다.
* **Prometheus & Grafana**: 애플리케이션의 핵심 지표(CPU, 메모리, 커넥션 풀 등)를 모니터링하고 시각화 대시보드를 구성합니다.
* **k6**: 가상 유저 시나리오 기반 부하 테스트를 설계하여 시스템 한계점 및 병목 구간을 진단합니다.
</details>

<br/>

## 📊 Github Stats
<p align="center">
  <img src="https://github-readme-stats-beryl-one-56.vercel.app/api?username=perandowind&show_icons=true&theme=radical" alt="Github Stats" />
  &nbsp;&nbsp;
  <img src="https://github-readme-stats-beryl-one-56.vercel.app/api/top-langs/?username=perandowind&layout=compact&theme=radical" alt="Top Langs" />
</p>

<br/>

## 📬 Contact
* **Email** : `your_email@gmail.com`
* **LinkedIn** : [https://www.linkedin.com/in/joon-choi-4715393ba/](https://www.linkedin.com/in/joon-choi-4715393ba/)
